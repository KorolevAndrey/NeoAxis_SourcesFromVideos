...
namespace Project
{
	public class PlayScreen : UIControl
	{
		...
		int ImageDir = -1;
		double ImageSpeed = 0.01;
		double ImageTime = 0;
		double ImageMaxTime = 1;
		bool isHide = false;
		...
		{
			if( EngineApp.ApplicationType == EngineApp.ApplicationTypeEnum.Simulation && ImageUpdate) {
				UpdateImage();
			}
		...
		private void UpdateImage() {
			...
			ImageTime = ImageTime + ImageSpeed;
			
			double transp = 0.0;
			if(ImageDir > 0) {
				transp = ImageTime / ImageMaxTime;
			} else {
				transp = 1.0 - ImageTime / ImageMaxTime;
			}
			//ScreenMessages.Add("UpdateImage ImageTime = " + ImageTime + ", transp = "+transp);
			img.BackgroundColor = new ColorValue( 0, 0, 0, transp);
			if(ImageTime > ImageMaxTime) {				
				if(ImageDir > 0) {
					img.BackgroundColor = new ColorValue( 0, 0, 0, 1 );
					isHide = true;
					//ScreenMessages.Add("Hide true");
				} else {
					img.BackgroundColor = new ColorValue( 0, 0, 0, 0 );
					isHide = false;
					//ScreenMessages.Add("Hide false");
				}
				ImageUpdate = false;
			}
		...
		public void Hide(double imageMaxTime) {	
			if(isHide == false) {
				ImageDir = 1;
				ImageTime = 0;
				ImageUpdate = true;
				ImageMaxTime = imageMaxTime;
				//ScreenMessages.Add("Hide");
			}
		}
		public void Show(double imageMaxTime) {	
			if(isHide == true) {
				ImageDir = -1;
				ImageTime = 0;
				ImageUpdate = true;
				ImageMaxTime = imageMaxTime;
				//ScreenMessages.Add("Show");
			}
		}
